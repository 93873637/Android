...
public class WaveSurfaceView extends SurfaceView implements SurfaceHolder.Callback{
	...
	private int mCanvasMiddleGridColor = CANVAS_MIDDLE_GRID_COLOR;
	...
	protected void initSurfaceView() {
		...
		mWavePaint.setColor(mWaveItemColor);
		mGridPaint = new Paint();
		mGridPaint.setColor(CANVAS_GRID_COLOR);
		mGridMiddlePaint = new Paint();
		mGridMiddlePaint.setColor(mCanvasMiddleGridColor);
		new Thread() {
			public void run() {
				initCanvas();
			}
	...
	public void setWaveItemColor(int color) {
		mWaveItemColor = color;
	}

	public void setCanvasMiddleGridColor(int color) {
		mCanvasMiddleGridColor = color;
	}
	...
	private void initCanvas() {
		...
		if (canvas == null) {
			...
			LogUtils.td("mCanvasCapacity = " + mCanvasCapacity);
			prepareCanvas(canvas);
			if (mWaveFile != null) {
				AudioUtils.loadWaveProfile(mWaveFile.getAbsolutePath(), mDataList, mCanvasCapacity);
				LogUtils.td("load data from " + mWaveFile.getAbsolutePath() + ", size = " + mDataList.size());
				if (hasData()) {
	 ...
	 * @param dataList:
	 */
	private void drawCanvas(Canvas canvas, List<Integer> dataList) {
		int canvasHeight = canvas.getHeight();
