...
abstract class AbstractUVCCameraHandler extends Handler {
	...
	public int readFrameCount() {
		...
		if (thread == null) {
			...
			int count = thread.mFrameCount;
			thread.mFrameCount = 0;
			return count;
		}
	}
	...
	public String getPreviewModeString() {
		switch (mWeakThread.get().getPreviewMode()) {
			case UVCCamera.FRAME_FORMAT_YUYV: return "YUYV";
			case UVCCamera.FRAME_FORMAT_MJPEG: return "MJPEG";
			default:
				return "UNKNONWN";
		}
	}
	...
	static final class CameraThread extends Thread {
		...
		public int getPreviewMode() {
			if ((mUVCCamera == null) || !mIsPreviewing)
				return -1;
			return mUVCCamera.getCurrentFrameFormat();
		}
		...
		public static String getPictureFileAbsolute() {
			File path = new File("/sdcard/multicam");
			if (!path.exists()) {
				path.mkdirs();
			}
			return "/sdcard/multicam/cap_" + System.currentTimeMillis() + ".jpg";
		}
		...
		public boolean savePicture(final ByteBuffer frame) {
			logd("savePicture: frame = " + frame.capacity());
			YuvImage image = new YuvImage(convert(frame), ImageFormat.NV21, mWidth, mHeight, null);
			...
			try {
				String fileAbs = getPictureFileAbsolute();
				File f = new File(fileAbs);
				if (f.exists() && !f.delete()) {
					loge("savePicture: delete old file \"" + fileAbs + "\" failed.");
					return false;
				}
				if (!f.createNewFile()) {
					loge("savePicture: create new file \"" + fileAbs + "\" failed.");
					return false;
				}
				...
				out.flush();
				out.close();
				logd("savePicture: frame saved to file \"" + fileAbs + "\" successfully.");
				return true;
			} catch (Exception e) {
				loge("savePicture: write to file failed, ex = " + e.toString());
				return false;
			}
		}
		...
		private boolean mCapFrame = false;

		private final IFrameCallback mIFrameCallback = new IFrameCallback() {
			@Override
			public void onFrame(final ByteBuffer frame) {
				mFrameCount ++;
				mTotalSize += frame.capacity();
				//logd("CameraThread@" + Integer.toHexString(hashCode()) + ": onFrame " + mFrameCount + "/" + frame.capacity() + "/" + mTotalSize);

				if (!mCapFrame) {
					if (savePicture(frame)) {
						mCapFrame = true;
						logi("TIME@: CameraThread@" + Integer.toHexString(hashCode()) + ": picture saved");

						//###@: close after save
						//CameraThread.this.handleClose();
					}
				}
