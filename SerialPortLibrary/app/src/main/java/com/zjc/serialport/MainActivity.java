...
public class MainActivity extends Activity implements View.OnClickListener, SerialCallBack {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.btn_open:
                boolean isOpen = SerialPortUtil.open("/dev/ttyUSB0", 115200, 0);
                if (isOpen) {
                    Log.e("TAG", "打开成功");
                } else {
                    Log.e("TAG", "打开失败");
            ...
            case R.id.btn_send:
                 ...
                 * 读取温湿度指令
                 */
                //byte[] bytes = {0x7E, 0x01, 0x00, 0x00};
                byte[] bytes = "0123456789".getBytes();
                SerialPortUtil.sendString(ByteUtil.getSum16(bytes, bytes.length));
                Log.e("TAG", "send data: thread = " + Thread.currentThread().getId());
                break;
            case R.id.btn_close:
    ...
    public void onSerialPortData(String serialPortData) {
        Log.e("TAG", Thread.currentThread().getId() + ": 来自串口的数据:" + serialPortData);
    }
}