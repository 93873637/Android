...
int main(int argc, char **argv)
{
	...
	/* main loop */
	in_tz_fd = open(tz_path, O_NONBLOCK,  S_IRUSR | S_IRGRP | S_IROTH);
	if (in_tz_fd < 0) {
		ALOGE("open tz %s fail error = %s\n", tz_path, strerror(errno));
		return -1;
	}
	...
	while(1) {
		...
		snprintf(tmp, PATH_MAX, "%s.%s.%d", out_path, TZ_LOG_NAME, curr_no);
		ALOGD("curr tz log file path=%s", tmp);

		out_tz_fd = open(tmp, O_WRONLY | O_CREAT | O_TRUNC, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
		if (out_tz_fd < 0) {
			ALOGE("open tz file fail error = %s\n", strerror(errno));
			return -1;
		}
		...
		while(1) {
			...
			read_tz_len = read(in_tz_fd, read_buf_tz, tz_BUF_LEN);
			//ALOGE(" in_tz_fd=0x%x  out_tz_fd=0x%x   read_buf_tz=0x%x read_tz_len=%d\r\n", in_tz_fd, out_tz_fd, read_buf_tz, read_tz_len);
			//ALOGE(" read_buf_tz=%s \r\n ", read_buf_tz);
			if (read_tz_len > 0) {
				write_tz_len = write(out_tz_fd, read_buf_tz, read_tz_len);
				if (time_left > 0)
					fsync(out_tz_fd);

				curr_size_tz += write_tz_len;
				if (curr_size_tz > size) {
					fsync(out_tz_fd);
					break;
				}
			} else {
