...
//        };
    ...
    //
    protected String checkNameByNumber(String callName, String callNumber) {
    ...
    public ArrayList<String> getCallHistoryList(Activity activity) {
        ...
        if (cs != null && cs.getCount() > 0) {
            for (cs.moveToFirst(); (!cs.isAfterLast()); cs.moveToNext()) {
                ...
                //add call name and number
                String callName = cs.getString(0);
                String callNumber = cs.getString(1);

                //if name empty, query contacts
                callName = checkNameByNumber(callName, callNumber);

                //add name and number
                callInfo += callName + " | " + callNumber + " | ";

                //add other items
                for (int i=2; i<cs.getColumnCount(); i++) {
